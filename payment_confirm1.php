...
if($amount_to_pay<$balance_amount)
{
 	...
 	$query = "INSERT INTO payment(userid,user_account,transfer_account,payment_type,amount,remark,date)VALUES('$_SESSION[user]','$account_no','ADMIN ACCOUNT','debit','$amount_to_pay','success','$date')";	
 	if(mysqli_query($con,$query))
	{
		$r5=mysqli_query($con,"SELECT MAX(bill_no+1) AS bill_no FROM saleschild");
	$row5=mysqli_fetch_array($r5);
	$bno=$row5['bill_no'];
	 mysqli_query($con,"UPDATE saleschild SET status='Purchased',bill_no='$bno',date='$date' WHERE user_id='$_SESSION[user]' and status='cart'");
	// echo "UPDATE saleschild SET status='Purchased',bill_no='$bno' WHERE user_id='$_SESSION[user]' and status='cart'";
	 $status="purchased";
    header("location:bill.php?bill_no=$bno");
	}
	else
	{
	$status="error";
	header("location:payment1.php?status=$status");
	}
}
else
{
